...
def validar_respuesta(letra,num_opciones):
    letras_opciones = ["A", "B", "C"][:num_opciones]
    if letra.isalpha() and len(letra) == 1 and letra in letras_opciones:
        return letra
    else:
...
def verificar_usuario_mayor(nombre):
    if (edad < 18):
        ...
        juego_trivia()
        # Al final del juego, se pregunta si desea jugar nuevamente
        muestra_mensaje_jugar_nuevamente(nombre)
...
#Función para mostrar el mensaje de jugar nuevamente y reiniciar el juego
def muestra_mensaje_jugar_nuevamente(nombre):
    jugar_nuevamente = input("¿Quieres jugar nuevamente? (S/N): ").upper()
